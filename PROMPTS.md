...
### 14. Correções de responsividade e ritmo do carrossel

**Objetivo:** o hero quebrava no celular (imagem repetida em faixas) e a troca de slides ficou lenta demais.

**Resultado verificado:** a panorâmica do hero animava `background-size` (112%→120%), o que solta o `cover`: num hero de celular com 1.500px de altura a imagem ficava com ~250px e o `background-repeat` a repetia seis vezes. A animação passou a ser de `transform` (escala + deslocamento, composta na GPU), com `background-size: cover` e `no-repeat` fixos; o espelhamento da arte do Pelé usa a propriedade `scale`, aplicada antes de `transform`, então a panorâmica dela inverte o sentido sozinha e um único keyframe serve aos três slides. Troca: 5,5s → 3,8s, crossfade 1,1s → 0,75s, panorâmica 12s. Na varredura de responsividade (11 páginas × 360/560/900/1024/1440px) apareceram mais dois problemas reais, ambos corrigidos: o header público estourava a janela entre 900 e 1120px (logo + seis links + "Entrar" + CTA somam ~1060px e o menu só colapsava abaixo de 900px) — agora a forma compacta vale até 1120px; e as grades `grid-cols-[repeat(N,1fr)]` não encolhiam abaixo do conteúdo, estourando em 900px exatos (19 ocorrências trocadas por `minmax(0,1fr)`, e os planos passam por 2 colunas antes de empilhar). Também no celular: os quatro números do hero ficam 2×2 (o hero caiu de 1.512 para 1.321px), a faixa de KPIs do mapa continua em três colunas com corpo menor, e links de rodapé, sumário da política e "Voltar à home" ganharam altura de toque. As tabelas do olheiro e da gestora já rolavam dentro de `.table-scroll`, conferido.

### 15. Alinhamento do login, separadores e ritmo das animações

**Objetivo:** três ajustes finos de revisão.

**Resultado verificado:** (1) No login, `.login-form > *` dava `max-w-form` **e** `mx-auto` a cada filho, mas o `mx-0` escrito no `<h2>` vencia e jogava o título e o aviso para a esquerda enquanto o resto centralizava. O limite de largura passou para o painel (`items-center` + filhos só com `max-w-form`) e os `mx-0` saíram do markup: abas, título, aviso, campos e rodapé agora começam todos em 846px. (2) Os tracinhos decorativos antes dos rótulos de seção (`<span class="w-6 h-px">` em manifesto, login e política) foram removidos, e o rótulo virou `.section-kicker` — 13px, mais forte, com um sublinhado que cresce quando o mouse entra na seção; as tags de abertura ganharam `.tag--section` (12px, mais respiro) e acendem no laranja da marca no hover da seção. Na mesma varredura, os travessões de prosa em texto visível viraram "·" ou pontuação normal: aviso do login, endereço em sobre, cinco rótulos da política, os onze nomes de peneira em `data.js` (com `eventShortName` aceitando os dois separadores) e os `<title>`/`og:title`. Ficaram de fora os travessões em comentários de código (internos) e o `—` de célula vazia nas tabelas, que ali significa "sem dado". (3) Animações mais lentas para dar tempo de ver: contagem dos números 0,9s → 1,8s, crescimento das barras 0,9s → 1,6s e a barra de completude do cadastro 0,6s → 1,0s.

### 16. Mapa de calor da gestora sobre a malha real

**Objetivo:** aproximar o mapa de calor da gestora do mapa interativo da página de peneiras, mantendo a ideia de calor e com cores que se expliquem sozinhas.

**Pedido:** na gestão, refazer o mapa de calor parecido com o de peneiras, mas de calor, mais próximo da ideia que já existia, com cores bem explicativas.

**Resultado verificado:** a tela usava bolhas posicionadas em percentual sobre um contorno decorativo pontilhado — as bolhas se sobrepunham em telas estreitas e a forma não era o Brasil. Agora é um choropleth sobre a mesma malha do mapa público (`brazil-map.js`), em `assets/js/mapa-calor.js`: cada estado é pintado pela **faixa de demanda** — baixa (verde, até 45%), média (âmbar, 45–65%), alta (laranja, 65–85%) e crítica (vermelho, acima de 85%) — com a opacidade variando dentro da faixa para dar o degradê sem perder a leitura por cor; estado sem dado fica cinza e não é clicável. Cobertura é outra dimensão e não cabia na mesma cor: estados com peneira aberta ganham contorno claro e um ponto. A legenda embaixo do mapa mostra a escala faixa a faixa com os percentuais, mais as duas marcas (peneira aberta, sem dado). Os quatro filtros atenuam o que está fora do recorte em vez de esconder — conferido: tudo 15/15 em foco, demanda alta 7, com peneira 6, demanda reprimida 4. Clique, Enter e Espaço selecionam (clicar de novo limpa), com tooltip no mouse e no teclado, `aria-label` por estado e `aria-pressed` no selecionado; a lista "Top demandas reprimidas" virou botões que selecionam o estado no mapa. O painel lateral passou a mostrar a faixa em que o estado caiu e a sugestão do sistema varia conforme demanda **e** cobertura. Os estados com peneira vêm de `MOCK.EVENTS` pelas mesmas funções do calendário (`eventStatus`), então a cobertura no mapa não diverge da lista; as UFs pequenas usam a mesma tabela de deslocamento do mapa público (DF já não escreve por cima de GO). `.map-grid` e `.map-dot`, do mapa antigo, saíram do `residual.css`.
