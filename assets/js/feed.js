...
const Feed = (() => {
  ...
  const hasVote = id => !!state.votes[id];
  ...
  const votesOf = a => baseVotes(a) + (hasVote(a.id) ? 1 : 0);
  ...
  function attrTagsHTML(a) {
    return Object.keys(a.attrs).map(attr => {
      ...
      return `<button type="button" class="attr-tag" data-tag="${esc(attr)}" data-athlete="${esc(a.id)}" aria-pressed="${on}">` +
        `<span>${esc(attr)}</span><span class="attr-tag__count" data-count>${tagCountOf(a, attr)}</span>` +
        `<span class="sr-only">confirmações</span></button>`;
    }).join('');
  }
  function voteButtonHTML(a) {
    const on = hasVote(a.id);
    return `<button type="button" class="btn btn--sm ${on ? 'btn--accent' : 'btn--ghost'}" data-vote data-athlete="${esc(a.id)}" aria-pressed="${on}">` +
      `<span aria-hidden="true">▲</span> <span data-count>${votesOf(a)}</span><span class="sr-only">votos</span></button>`;
  }
  ...
  function updateCounts(root, id) {
    ...
    UI.qsa(`[data-vote][data-athlete="${id}"]`, root).forEach(b => {
      const on = hasVote(id);
      UI.setPressed(b, on);
      b.classList.toggle('btn--accent', on); b.classList.toggle('btn--ghost', !on);
      UI.qs('[data-count]', b).textContent = votesOf(a);
    });
  ...
  function mount(root) {
    ...
    function applyFilters(announceResult) {
      ...
      empty.hidden = n > 0;
      if (announceResult) announce(`${n} ${n === 1 ? 'atleta' : 'atletas'} no feed`);
    }
